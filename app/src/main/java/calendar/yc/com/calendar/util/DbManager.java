...
import calendar.yc.com.calendar.bean.IndexDate;
...
public class DbManager {
    ...
    public static void initManager(Context context) {
        if (mInstance == null) {
            ...
            databasepath = SDCardUtils.getSDCardPath() + "/calendar/%s/db";
        }
    }
    ...
    public SQLiteDatabase getDatabase() {
        if (databases.get(dbName) != null) {
            RxLogUtils.i(tag, String.format("Return a database copy of %s", dbName));
            return (SQLiteDatabase) databases.get(dbName);
        }
        if (context == null)
            return null;
        RxLogUtils.i(tag, String.format("Create database %s", dbName));
        ...
        if (!flag || !file.exists()) {
            ...
            if (!file.exists() && !file.mkdirs()) {
                RxLogUtils.i(tag, "Create \"" + spath + "\" fail!");
                return null;
            }
            if (!copyAssetsToFilesystem(dbName, sfile)) {
                RxLogUtils.i(tag, String.format("Copy %s to %s fail!", dbName, sfile));
                return null;
            }
    ...
    private boolean copyAssetsToFilesystem(String assetsSrc, String des) {
        RxLogUtils.i(tag, "Copy " + assetsSrc + " to " + des);
    ...
    public static void closeAllDatabase() {
        RxLogUtils.i(tag, "closeAllDatabase");
        if (mInstance != null) {
...
//                sqliteDB.close();
    ...
    public HuangLiDbInfo getYiJiInfo(String date) {
        ...
        sb.append("-");
        if (!dates[1].startsWith("0") &&Integer.parseInt(dates[2]) < 10) {
