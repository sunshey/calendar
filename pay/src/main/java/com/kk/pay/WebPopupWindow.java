...
public class WebPopupWindow extends PopupWindow {
    ...
    public WebPopupWindow(Activity context, final String url, final boolean isOverloadUrl) {
        mContext = context;
    ...
    private boolean openWxpay(String url) {
        ...
        if (url.startsWith("weixin://")) {
            try {
                ...
                e.printStackTrace();
                if (!JudgeUtil.checkWxPayInstalled(mContext)) {
                    ToastUtil.toast2(mContext, "你的手机没安装微信，请先安装！");
                } else {
